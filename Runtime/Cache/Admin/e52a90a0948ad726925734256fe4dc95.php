    ...
    <div class="data-table table-striped">
        <table class="">
            ...
            <?php if(!empty($_apply_list)): if(is_array($_apply_list)): $i = 0; $__LIST__ = $_apply_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <?php if($vo['status'] == 2): ?><span style="font-size: 14px;font-weight: bold;color: white;background-color: #5992CB;border-radius: 3px;padding: 5px;">已处理</span>
                                <?php else: ?>
                                <a  style="font-size: 14px;font-weight: bold;color: white;background-color: #5992CB;border-radius: 3px;padding: 5px;" href="<?php echo U('User/deal?id='.$vo['id'].'&user_id='.$vo['user_id']);?>" class="sell confirm ajax-get">处理</a><?php endif; ?>
                        </td>
