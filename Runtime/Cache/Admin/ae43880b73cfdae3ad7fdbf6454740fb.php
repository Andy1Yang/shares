...
<body>
    ...
    <form action="<?php echo U();?>" method="post" class="form-horizontal">
        <div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text input-large" name="name" value="<?php echo ($_res["name"]); ?>">
                <input type="hidden" class="text input-large" name="is_eidt" value="1">
                <input type="hidden" class="text input-large" name="id" value="<?php echo ($_res["id"]); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">重置密码</label>
            <div class="controls">
        ...
        <div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text input-large" name="pledge" value="<?php echo ($_res["pledge"]); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">利率<span class="check-tips">（单位：毫/天）</span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="rate" value="<?php echo ($_res["rate"]); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">佣金费率 <span class="check-tips">（单位：‰）</span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="yongjin_rate" value="<?php echo ($_res["yongjin_rate"]); ?>">
            </div>
        </div>
        <!--div class="form-item">
            <label class="item-label">资金预警<span class="check-tips">（单位：％）</span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="capital_waring" value="<?php echo ($_res["capital_waring"]); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">账户平仓<span class="check-tips">（单位：％）</span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="flat" value="<?php echo ($_res["flat"]); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">收过户费<span class="check-tips">（单位：元/千股）</span></label>
            <div class="controls">
                <input type="text" class="text input-large" name="transfer"  value="<?php echo ($_res["transfer"]); ?>">
            </div>
        </div>
        <div class="form-item">
            <span class="item-label" style="display: inline;">月底返息:</span>
            <span class="controls">
                是<input type="radio" name="return_rate" value="1" <?php if(($_res['return_rate'] == 1)): ?>checked="true"<?php endif; ?> >&nbsp;
                否 <input type="radio" name="return_rate" value="2"  <?php if(($_res['return_rate'] == 2)): ?>checked="true"<?php endif; ?>>
            </span>
        </div>
        <div class="form-item">
            <span class="item-label" style="display: inline;">收手续费:</span>
            <span class="controls">
                是<input type="radio" name="hand_cost" value="1"  <?php if(($_res['hand_cost'] == 1)): ?>checked="true"<?php endif; ?>>&nbsp;
                否 <input type="radio" name="hand_cost" value="2"  <?php if(($_res['hand_cost'] == 2)): ?>checked="true"<?php endif; ?>>
            </span>
        </div-->
        <div class="form-item">
            ...
            <div class="controls">
                <select name="sales_id" id="">
                    <?php if(is_array($_sales_list)): $i = 0; $__LIST__ = $_sales_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sa): $mod = ($i % 2 );++$i;?><option value="<?php echo ($sa["id"]); ?>" <?php if(($_res['sales_id'] == $sa['id'])): ?>selected='selected'<?php endif; ?>><?php echo ($sa["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
        <div class="form-item">
        ...
        <div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text input-large" name="body_card" value="<?php echo ($_res["body_card"]); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">手机号</label>
            <div class="controls">
                <input type="text" class="text input-large" name="telephone" value="<?php echo ($_res["telephone"]); ?>">
            </div>
        </div>
        <div class="form-item">
            <label class="item-label">手机号</label>
            <div class="controls">
                <input type="text" class="text input-large" name="telephone2" value="<?php echo ($_res["telephone2"]); ?>">
            </div>
        </div>
        <div class="form-item">
            <span class="item-label" style="display: inline;">性别:</span>
            <span class="controls">
                男<input type="radio" name="sex" value="1" <?php if(($_res['sex'] == 1)): ?>checked="true"<?php endif; ?> >&nbsp;
                女<input type="radio" name="sex" value="2" <?php if(($_res['sex'] == 2)): ?>checked="true"<?php endif; ?> >
            </span>
        </div>
        <!--div class="form-item">
            <label class="item-label">电话</label>
            <div class="controls">
                <input type="text" class="text input-large" name="phone" value="<?php echo ($_res["phone"]); ?>">
            </div>
        </div-->
        <div class="form-item">
        ...
        <!--div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text input-large" name="sort" value="<?php echo ($_res["sort"]); ?>">
            </div>
        </div-->
        <div class="form-item">
    ...
    <script type="text/javascript">
        //导航高亮
        highlight_subnav('<?php echo U('User/user_list');?>');
    </script>
